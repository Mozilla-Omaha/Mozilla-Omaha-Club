...
if(has_JSON_Format === true) return JSON.parse(this);
	
var _emptyStringValue = this.replace( /\'\'(?=\,)|\'\'(?=\})|\'\'(?=\:)|\'\'\s+\'\'(?=\,)|\'\'\s+\'\'(?=\})|\'\'\s+\'\'(?=\:)/g,'"null"' ),

	_singleToDoubleQoutes = _emptyStringValue.replace(/\'+/g,'"'),
	
	_fixNullType = _singleToDoubleQoutes.replace(/null+(?=\"\w)|null+(?=\s+)/g, 'null,' ),
	
	_space = _fixNullType.replace( /\'\'\s+\:\s+\'\'|\'\'\s+\:\s+\[|\'\'\s+\:\s+\{|\'\'\s+\:\s+\d+|\'\'\s+\:\s+\.\d+/g, "" ),
	
	_fixStringType = _space.replace(/\'\'(?=\'\'\w+\'\'\s+)|\'\'+(?=\w+\s+)|\'\'+(?=\'\'\w+\'\')|\'\'+(?=\w+)/g, '",' ),
