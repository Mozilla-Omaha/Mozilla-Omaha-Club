...
/********* Notes:  The applyMixins() function used in the xtag.register() method. **********/
...
var item = mixin[type],
	...
	tag[type] = item;
				...
				switch (type){
					...
					case 'events': addMixin(tag, original, item); 
					break;
...
/* ***** Notes: Addition to xtag object.
	Added the json property. 
***** */
  var xtag = {
    tags: {},
    defaultOptions: {
...
/* ********************
	Notes: Start of xtag.register()

   ******************** */
    register: 
		function (name, options) {

/* Notes: Check object type of the name parameter and set the _name variable. */
var _name = typeof name == "string" ? 
		( _name = name.toLowerCase() ) :
		( _name = 'First argument must be a Custom Element string name' );
...
/* Check tag for custom attributes. */
...
var attr = tag.attributes[ name ],
	...
	this[ attr.key ] = attr.boolean ? 
						hasAttr : 
							!hasAttr && hasDefault ? 
								attr.def : this.getAttribute( name );
	...
	definition['prototype'] = Object.create(
		extended ? 
			Object.create( doc.createElement( extended ).constructor ).prototype : 
				instance ? 
					basePrototype : win.HTMLElement.prototype, tag.prototype )
...
/* ***** Notes: base.getElement( identifier_list )
	Tool used to retrieve HTML element node lists. 
	'#' : get element id, '.' : get element class,  '$' : get element name, '' : get element tag
***** */
	getElement: function( identifier_list ){
var matches = identifier_list.replace(/s+/g,"").toString().match(/\w+\,+|\w+\;/g),
	matchedElements = [];

		switch( identifier_list[0] ){
			case "#":

	for( var i=0; i < matches.length ; i++ ){

		matchedElements.push( document.getElementById( matches[i] ) );
	...
	for( var i=0; i < matches.length ; i++ ){
		...
		matchedElements.push( document.getElementsByClassName( matches[i] ) );
	...
	for( var i=0; i < matches.length ; i++ ){
		...
		matchedElements.push( document.getElementsByName( matches[i] ) );
	...
	for( var i=0; i < matches.length ; i++ ){
		...
		matchedElements.push( document.getElementsByTagName( matches[i] ) );
...
/* ***** Notes: base.getElement( identifier_list )
	...
	'#' : get element id, '.' : get element class,  '$' : get element name, '' : get element tag
***** */
	getJsonByProperty: Array.prototype.getJsonByProperty = function( key, value ){
var matched = [];

		for( var i=0; i < base.json.length; i++){

				if( typeof base.json[i] == "object" && base.json.length > 0 ){

	base.json[i][key] === value ? 
		matched.push(base.json[i]) : "No property match found";
...
/* ********** Notes:  ********** */
	_toJSON: String.prototype._toJSON = function( has_JSON_Format ){
var _jsonObject;

	/* ** First check to see if the string is already in JSON format ** */
		if( has_JSON_Format === true ){ 

	_jsonObject = JSON.parse(this); 
	...
	/* ** If has_JSON_Format is false run the rest of the program ** */	
var _emptyStringValue = this.replace( /\'\'(?=\,)|\'\'(?=\})|\'\'(?=\:)|\'\'\s+\'\'(?=\,)|\'\'\s+\'\'(?=\})|\'\'\s+\'\'(?=\:)/g,'"null"' ),
	...
	_fixLiteralNotation = _fixNullType.replace(/\w+(?=\:)/g, function( string ){ return '"' + string + '"' } ),
	
	_space = _fixLiteralNotation.replace( /\s+(?=\"\w+\"\:)|\s+(?=\[)|\s+(?=\{)|\s+(?=\])|\s+(?=\})/g, "" ),
	
	_lineBreak = _space.replace( /\n+| +/g, " " ),

	_fixStringType = _lineBreak.replace(/\'\'(?=\'\'\w+\'\'\s+)|\'\'+(?=\w+\s+)|\'\'+(?=\'\'\w+\'\')|\'\'+(?=\w+)/g, '",' ),
...
/* ********** Notes:  JTML (JSON Text Markup Language ********** */
	JTML: String.prototype.JTML = function( dataJSON ){
var _rootClass = "", _childrenClass = "";

	/* ********** Notes: Start iteration through the string object. ********** */
		for( var i=0; i < this.length; i++ ){

	/* ********** Notes:   ********** */
				if(/\(/.test( this ) === true ){
	_pallete.className = _rootClass;
	_pallete.id = "";
	_rootClass = "";
					break;
				}
				else{
					_rootClass += _drawingTypeJson[i].type[x];
				}
...
/* ********** Notes: AKA the XTAG_REGISTRATION Update Loop.  ********** */	
	update: function( xtag_opts ){

		for( var i=0; i < xtag_opts.length; i++ ){

var x_reg = xtag.register( xtag_opts[i][0], {
		lifecycle: {
			created: function(){
		...
		accessors: 
		{
			dataJson: {
				attribute: {
					validate: function( html_json ){

	html_json._toJSON( false );
...
var node = base.getElement,
	getProp = base.getJsonByProperty;
